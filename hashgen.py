...
def hashgen_file(filename, buffsize, quiet):
...
def sha1_write(path, name, buffer, quiet):
    sha1sum = hashgen_file(Path(f"{path}\\{name}"), buffer, quiet)
    with open(Path(f"{path}\\{name}.sha1"), 'w', encoding="utf8") as sha1file:
        sha1file.write(f"{sha1sum}   {name}")  # ← 3 spaces!
    return sha1sum

def exist_files_check(src, dst, file_list, buffer, quiet):
    """
    ...
    """
    def dict_mount(path, name, buffer, quiet, need_update):
        try:
            with open(Path(f"{path}\\{name}.sha1"), 'r') as s:
                string = s.readline()
        except FileNotFoundError:
            return sha1_write(path, name, buffer, quiet) if need_update else 0
        return string.split("   ")[0]  # ← 3 spaces!

    src_dict = {}
    dst_dict = {}
    for file in file_list:
        src_dict[file] = dict_mount(src, file, buffer, quiet, True)
        dst_dict[file] = dict_mount(dst, file, buffer, quiet, False)
...
if __name__ == '__main__':
    ...
    if args.sync or args.fullsync:
        ...
        dest_list = exist_files_check(args.source, args.destination, source_list[0], buffer_size, args.quiet)
        if dest_list:
            for x in dest_list:
                for_copy.append(x)
